...
public class XListView extends ListView implements OnScrollListener {
    ...
    private void updateHeaderHeight(float delta) {
        ...
        if (mEnablePullRefresh && !mPullRefreshing)
            ...
            if (mHeaderView.getVisiableHeight() > mHeaderViewHeight)
                mHeaderView.setState(PullLoadMoreListView.STATE_READY);
            else
                mHeaderView.setState(PullLoadMoreListView.STATE_NORMAL);
    ...
    public void refresh(OnXListViewListener mListViewListener) {
        ...
        mHeaderView.setState(PullLoadMoreListView.STATE_REFRESHING);
        if (mListViewListener != null)
            mListViewListener.onRefresh();
        if ((getAdapter() == null || getAdapter().getCount() < 1) && getEmptyView() != null
                && getEmptyView() instanceof EmptyView) {
